sampleSigma<-function(N,Npred,a,b){  ###This function will return variance. Need to sqrt to get SD
  n<-length(c(N))
  tmp.a<-a+n/2
  tmp.b<-b+(0.5*sum((N-Npred)^2))
...
UpdateBeta<-function(tmax,a0,b0,N,M,p){ # add X and C to list of arguments
...
# }
# dispersal
UpdateDispersal<-function(tmax,tau,N,G,p,D){
  Npred<-matrix(NA,tmax,p)
  Mint<-exp(-(D/tau))
