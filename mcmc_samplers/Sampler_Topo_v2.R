...
pmax<-dim(N)[2]
D<-Dsq
X<-enviro.var[,,4:5] # covariates this is where they go (1 = ppt, 2 = tmean, 3 = vpdmax, 4 = heatload, 5 = elev)
amax<-3 # number of env covariates +1
bmax<-1
...
alpha0<-0.003 ###Give beta some starting values based on what we know
alpha1<-0.001 # regression coef for heatload
alpha2<-0.001 # regresstion coef for elev
beta0<-0.0007
...
# *** using updateBetaClim function but for topographic covariates here
for (i in 1:Niter){
